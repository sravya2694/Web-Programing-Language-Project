	...
	<div class="col-md-7">
		...
		<div >
			...
			<div>
				...
				$que_all_posts = mysqli_query($con,"select * from user_post order by post_id desc");
				$que_friends = mysqli_query($con,"select friend_id from user_friends where user_id = $user_id");
				$que_fof = mysqli_query($con,"select friend_id from user_friends where user_id in (select friend_id from user_friends where user_id = $user_id)");
				$que_friends_arr = mysqli_fetch_array($que_friends);
				$que_fof_arr = mysqli_fetch_array($que_fof);
				
				$que_post = mysqli_query($con,"
				select * from user_post where user_id in (
					select friend_id from user_friends where user_id = $user_id) or user_id = $user_id		
				order by post_id desc;
				");
				
					while($post_data=mysqli_fetch_array($que_all_posts))
					{
						$post_priority = $post_data[5];
						...
						$post_priority = $post_data[5];
						$post_to_user_id = $post_data[6];
						$friend_id = $post_data[1];
						...
						if((mysqli_num_rows($que_friends)==0) && (mysqli_num_rows($que_fof)==0) )
						{
							
							if( $post_user_id == $user_id || $post_to_user_id == $user_id)
							{
								goto proceed;
							}
							else
						...
						else if ((!(mysqli_num_rows($que_friends)==0)) && (mysqli_num_rows($que_fof)==0) )
						{
							
							if($post_priority == 'Public' || $post_priority == 'Friends' || $post_priority == 'FOF')
							{
								
								if(!($post_user_id == $user_id || $post_to_user_id == $user_id || in_array($post_user_id , $que_friends_arr)))
									continue;
							}
							else if($post_priority =='Private')
							{
						...
						else
						{
							...
							else if($post_priority == 'FOF')
							{
								if(!($post_user_id == $user_id || $post_to_user_id == $user_id || in_array($post_user_id , $que_fof_arr)))
									continue;
							}
						}
