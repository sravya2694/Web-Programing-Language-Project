...
<?php
	if( isset($_POST['file']) && ($_POST['file']=='post'))
	{
		...
		if(isset($_POST['file']))
		{
			if($userInSession != $user_id)
			{
				$path1 = "../users/".$userInSession."/Notification/";
				$img_name=$_FILES['file']['name'];
				$img_tmp_name=$_FILES['file']['tmp_name'];
				$prod_img_path=$img_name;
				move_uploaded_file($img_tmp_name,$path1.$prod_img_path);
			}
			else
			{
		...
		{
			$notify_status = 0;
			mysqli_query($con,"insert into user_notification(from_user_id,to_user_id,post_txt,post_pic,post_time,priority) values('$userInSession','$user_id','$txt','$img_name','$post_time','$priority');");
		}
		else
		{
			mysqli_query($con,"insert into user_post(user_id,post_txt,post_pic,post_time,priority,to_user_id) values('$userInSession','$txt','$img_name','$post_time','$priority','$userInSession');");
		}
	}
	if(isset($_POST['commenting']))
	{
		...
		mysqli_query($con,"insert into user_post_comment(post_id,user_id,comment,comment_pic) values($post_id,'$userInSession','$txt','$img_name');");
	}
	if(isset($_POST['Like']))
	{
		$post_id=intval($_POST['postid']);
		$user_id=intval($_POST['userid']);
		mysqli_query($con,"insert into user_post_status(post_id,user_id,status) values($post_id,$userInSession,'Like');");
	}
	if(isset($_POST['Unlike']))
	{
		$post_id=intval($_POST['postid']);
		$user_id=intval($_POST['userid']);
		mysqli_query($con,"delete from user_post_status where post_id=$post_id and  	user_id=$userInSession;");
	}
	if(isset($_POST['comment']))
	{
		...
		if($txt!="")
		{
		mysqli_query($con,"insert into user_post_comment(post_id,user_id,comment) values($post_id,$userInSession,'$txt');");
		}
	}
	...
	<div class="col-md-7">
		...
		<div >
			...
			<div>
				<table cellspacing="0">
				<?php
					$que_post=mysqli_query($con,"select * from user_post where (user_id=$user_id or to_user_id=$user_id) and priority='public' order by post_id desc");
				...
				?>
					<video width="80%" controls>
						<source src ="./<?php echo $comment_user_id; ?>/Comment/<?php echo $comment_img; ?>" type='video/<?php echo $comment_extension; ?>' >
					</video>
				<?php
					}
					else
					{
				?>
					<img src="./<?php echo $comment_user_id; ?>/Comment/<?php echo $comment_img; ?>" style="max-width:100%; height:auto;"> 
				<?php
					}
					}
				?>
				...
				<tr>
					...
					<?php
					$query4 = mysqli_query($con,"select * from user_profile_pic where user_id = $userInSession;");
					...
					<td colspan="2" style="padding-top:15;"> 
						<form method="post" enctype="multipart/form-data" name="commenting" onSubmit="return blank_comment_check()" id="cmnt"> 
							...
							<input type="hidden" name="postid" value="<?php echo $postid; ?>"> 
							<input type="hidden" name="userid" value="<?php echo $user_id; ?>"> 
							<input id="post_img" type="file" name ="file" style="width:50%" align="right"/>
