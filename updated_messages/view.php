...
<body>
	...
	<div id=body>
		...
		<div class="container">
			<div class="row">
				<div class="col">
					<button type="button" class="btn btn-primary"><a href="messages.php" style="color: white">Inbox</a></button><br><br>
				...
				<div class="col-10 border-top">
					...
					<?php
					// Create connection
					$find = "SELECT * FROM user_messages WHERE message_id='$message_id'";
					$result = mysqli_query($conn,$find);

					while($results = mysqli_fetch_array($result)){

						echo nl2br($results['thread']);
								...
								<select name="recipient">
									...
									while($row = mysqli_fetch_array($results)) {
										...
										while($row2 = mysqli_fetch_array($name)) {
											echo '<option value='.$row['friend_id'].' id='.$row['friend_id'].'>'.$row2['Name'].'</option>';
										}
									}
									?>
								</select>
						...
						<div class="modal-body">
							<form action="reply_to_thread.php" method="post">
								...
								<select name="thread">
									...
									while($row = mysqli_fetch_array($results)) {
										echo '<option value='.$row['message_content'].' id='.$row['message_id'].'>'.$row['message_content'].'</option>';
									}
									?>
								</select>
								<br><br>
						...
						<div class="modal-body">
							<form action="reply_to_thread.php" method="post">
								...
								<select name="thread">
									...
									while($row = mysqli_fetch_array($results)) {
										echo '<option value='.$row['message_content'].'>'.$row['message_content'].'</option>';
									}
									?>
								</select>
								<br><br>
